...
def gerar_inventario(dados: dict = None, output_path=None):
    _dados = dados or {}
    ...
    _ibp_geral    = _dados.get('ibp_geral')
    _ibp_subcats  = _dados.get('ibpSubcats') or {}
    _num_resp     = _dados.get('respondentes', 0)

    # Gera inventario a partir de subcategorias criticas (ibp <= -1.5 = Sofrimento Patogenico)
    _inventario_dinamico = []
    por_unidade = _dados.get('porUnidade') or []
    setores_unicos = list(dict.fromkeys(u.get('unidade','Geral') for u in por_unidade)) or ['Empresa (Geral)']
    for setor in setores_unicos:
        ibp_setor = next((u['ibp'] for u in por_unidade if u.get('unidade') == setor), _ibp_geral or 0.0)
        perigos = []
        for sc_id, sc in _ibp_subcats.items():
            if sc.get('ibp', 0) <= -1.5:
                perigos.append({
                    'perigo': f"Risco critico em {sc.get('nome', sc_id)}",
                    'subcategoria': sc.get('nome', sc_id),
                    'origem': 'pesquisa'
                })
        _inventario_dinamico.append({'setor': setor, 'ibp': ibp_setor, 'perigos': perigos})
    ...
    story.append(Paragraph("Inventario de Riscos Psicossociais", s_h1))
    story.append(Paragraph(
        f"{_empresa_nome} · Referencia: {_referencia} · Documento para anexacao ao PGR (NR-1) — "
        f"1 instrumento do fluxo GRO (Inventario → Avaliacao → Plano de Acao → Acompanhamento)",
        s_sub
    ))
    story.append(Paragraph(
