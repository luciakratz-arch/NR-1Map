...
def gerar_mapa_risco(dados: dict = None, output_path=None):
    ...
    def desenhar_cabecalho_rodape_local(canvas_obj, doc):
        ...
        if _lp_loc:
            try: canvas_obj.drawImage(_IR(_lp_loc), 18*mm, h-28*mm, width=50*mm, height=14*mm, preserveAspectRatio=True, mask='auto')
            except:
        ...
        if _le_loc:
            try: canvas_obj.drawImage(_IR(_le_loc), w-62*mm, h-28*mm, width=50*mm, height=14*mm, preserveAspectRatio=True, mask='auto')
            except: pass
        canvas_obj.setFont('Helvetica-Bold', 11)
        canvas_obj.setFillColor(VERDE_NR1)
